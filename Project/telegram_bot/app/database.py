    ...
    try:
        async with db_pool.acquire() as connection:
            await connection.execute(
                """
                INSERT INTO feedback_bot.feedback (username, feedback, message_time, user_id)
                VALUES ($1, $2, $3, $4);
                """,
...
async def fetch_data():
    ...
    try:
        async with db_pool.acquire() as connection:
            rows = await connection.fetch("SELECT username, feedback, message_time FROM feedback_bot.feedback;")
